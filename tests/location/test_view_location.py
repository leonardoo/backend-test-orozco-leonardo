...
def test_location_edit_get(client, create_user, create_location):
    ...
    user = create_user(is_staff=True)
    client.force_login(user)
    response = client.get(reverse_lazy('location:country_edit', kwargs={'pk': location.pk}))
    assert response.status_code == 200


@pytest.mark.django_db
def test_dont_allow_user_not_staff_to_get_edit_location(client, create_user, create_location):
    """
    GIVEN a Flask application
    WHEN the '/profile/create' page is requested
    THEN check the response is valid
...
def test_location_create_post(client, create_user, create_location):
    ...
    user = create_user(is_staff=True)
    client.force_login(user)
    response = client.post(reverse_lazy('location:country_create'), data={
        'name': "Test Country",
        'tz': 'America/Bogota'
    }, follow=True)
    assert response.status_code == 200
    assert Country.objects.exclude(pk__in=[location.pk for location in create_location]).count() == 1
    country = Country.objects.exclude(pk__in=[location.pk for location in create_location]).first()
...
def test_location_edit_post(client, create_user, create_location):
    ...
    user = create_user(is_staff=True)
    client.force_login(user)
    response = client.post(reverse_lazy('location:country_edit', kwargs={"pk": location.pk}), data={
        'name': "Test Country",
        'tz': 'America/Bogota'
    }, follow=True)
    assert response.status_code == 200
